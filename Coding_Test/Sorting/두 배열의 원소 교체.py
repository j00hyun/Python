...
#   입력 조건: 첫 번째 줄에 N, K가 공백을 기준으로 구분되어 입력됩니다. (1 <= N <= 100,000, 0 <= K <= N)
...
#   출력 조건: 최대 K번의 바꿔치기 연산을 수행하며 만들 수 있는 배열 A의 모든 원소의 합의 최댓값을 출력합니다.
#   입력 예시: 5 3
#            1 2 5 4 3
#            5 5 6 6 5
#   출력 예시: 26
...
##### 내 풀이
arr_len, max_switch = map(int, input().split()) # 배열 길이, 최대 바꿔치기 연산 가능 횟수
arr_a = list(map(int, input().split())) # 배열 A
arr_b = list(map(int, input().split())) # 배열 B

arr_a.sort() # 배열 A 오름차순 정렬
arr_b.sort(reverse=True) # 배열 B 내림차순 정렬

# 배열 첫 번째 원소부터 차례대로 탐색
for i in range(arr_len):
  # 최대 바꿔치기 연산 가능 횟수를 초과하거나 배열 A의 원소 값이 배열 B의 원소 값보다 같거나 크다면 바꿔치기 종료
  if i >= max_switch or arr_a[i] >= arr_b[i]: 
    break
  # 그렇지 않다면 바꿔치기 수행
  else:
