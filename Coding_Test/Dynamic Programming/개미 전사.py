...
foods = list(map(int, input().split())) # 각 식량창고에 저장된 식량의 개수
result = 0 # 식량의 최댓값을 저장할 변수

# 재귀함수
def recur(i, f_sum):
  global result
  f_sum += foods[i] # 현재 도착한 식량창고에 있는 식량을 가방에 추가

  if i + 2 >= n: # 현재 식량창고에서 더이상 더 갈 수 없다면
    if f_sum > result: # 가방에 있는 값이 현재까지의 최댓값과 비교해 최댓값 갱신 후 종료
      result = f_sum 
      return

  if i + 2 < n: # 현재 식량 창고에서 2칸 떨어진 곳으로 이동
    recur(i + 2, f_sum)
  if i + 3 < n: # 현재 식량 창고에서 3칸 떨어진 곳으로 이동
    recur(i + 3, f_sum)
